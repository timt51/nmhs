% Hodgkin-Huxley circuit model implemented by Leif Gibb 6/28/15 through 8/10/15
...
function ydot = hh_ode(t,y,Nneur,Nsyn,time,Iin,gsyn)
    ...
    for j = 1:Nsyn
        ...
        r(j) = y(4*Nneur + j);
        rdot(j) = alphasyn*T(j)*(1-r(j)) - betasyn*r(j);
        Isyn(j) = gsyn(j)*r(j)*(Vpost(j)-Erev(j));
    end
    ...
    ydot = [udot(1); mdot(1); ndot(1); hdot(1); udot(2); mdot(2); ndot(2); hdot(2); rdot(1)];
