...
function ydot = hh_ode(t,y,Nneur,Nsyn,time,Iin,gsyn)
    ...
    ydot = zeros(4*Nneur+Nsyn,1);
    for i = 1:Nneur
        ydot(4*(i-1)+1,1) = udot(i);
        ydot(4*(i-1)+2,1) = mdot(i);
        ydot(4*(i-1)+3,1) = ndot(i);
        ydot(4*(i-1)+4,1) = hdot(i);
    end
    for j = 1:Nsyn
        ydot(4*Nneur+j,1) = rdot(j);
    end
